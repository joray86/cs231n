...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i,hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = np.random.randn(input_layer_dim, hd) * weight_scale
            self.params['b%d'%(i+1)] = np.zeros(hd)
            input_layer_dim = hd
        self.params['W%d'%(self.num_layers)] = np.random.randn(input_layer_dim, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
           a[i+1],cache[i+1] = affine_relu_forward(input_layer, self.params['W%d'%(i+1)], self.params['b%d'%(i+1)])
           input_layer = a[i+1]
        a[self.num_layers],cache[self.num_layers] = affine_forward(input_layer, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        ...
        for i in range(self.num_layers-1)[::-1]:
          ...
          dx, dw, db = affine_relu_backward(dout , cache[i+1])
